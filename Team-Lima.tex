...
\subsubsection{4.1.1 Architectural Scope}

\begin{itemize}
	\item Access to the data from anywhere at any time ie. a database that is always running.
	\item A server to host the database that is always on.
	\item Persistent internet connection that has a backup to allow people to always be able to have access to the server.
	\item The ability to let users know via email about important notifications and so forth.
	\item A progress report on each project ie. how much work has been completed on the project.
	\item Setting of deadlines on research projects. (Perception of time)
	\item Concurrency: to allow 100 concurrent users to use the system at the same time.
	\item Authentication with University of Pretoria servers to validate the users staff/student number.
	\item Super user access for the Head Of Department
	\item Write permissions for files and log files
	\item Searching capabilities
\end{itemize}

\subsubsection{4.1.2 Quality Requirements}
...
\item \subsubsection{Reliability}
...
\item \subsubsection{Flexibility}
...
\item \subsubsection{Maintainability}
...
\item \subsubsection{Monitorability}
...
\item \subsubsection{Integrability}
...
\subsubsection{4.1.3 Integration And Access Channel Requirements}


 
\subsubsection{4.1.4 Architecture constraints}
...
\begin{itemize}
\item \subsubsection{Web-based development}
...
\item \subsubsection{Server-side development}
...
\item \subsubsection{Mobile application development}
...
\end{itemize}
	%Going to check the domain model and see what layer should do what
...
\item Bound Execution time, in our system we need a way of limiting the execution time spent on each event so that resources can free up to allow other events to run.
...
\item The system will treat all external requests as possible threats and not allow them onto the system, this will narrow th direction of where we recieve the threats from making it easier to detect the threats to the system.

\item Maintain and check for attack signatures, so that we will know exactly where the threats are coming from, so basically we are to keep all trails in the system(trace actions of the attacker).

\item To recover from attacts, we will drop all requests on the system with the intention of basically refrshing the system

\item  After dealing the threats on the system, the system will have to be restored, and resume basic functionality.

\item Authenticating all users before they log in is another tactic, after this based on the kind of user logging into the system, user authorization will be performed to make sure user doesnt have access to what they arent meant to have access to. The users will have to enter they passwords to authenticate them, to maintain the systems integrity all passwords will be hashed and stored in the database and the system will need a way of encrypting all data and communication links.
...
\begin{itemize}
	...
	\item Manage the input and output on the system.
	\item Have a internal monitor that is built-in that is always running to keep track and log all user activitys. We will use a tactic called record and playback which is basically recieving infomation across an interface and using that infomation as input to test harness.
...
\begin{itemize}
\item When the system is under heavy workload, and cannot process all requests, to restore the speed old request that have been running for some time will be terminated and user will have to make another request.

\item Increase disc space, this will help the system to handle more requests
...
\subsubsection{Java-EE Reference Architecture}
...
Component at first level of granularity.

\textbf{Pattern}: Layering
\begin{itemize}
\item Access layer: web container,message queues
\item Business processes layer: EJB container
\item Persistence layer: Persistence context
\end{itemize}

\textbf{Tactics}:
Clustering for scalability,availability and reliability
\begin{itemize}
\item All layer elements
\item JNDI repositories
\item Load balancers, firewalls,DBs
\end{itemize}
JMX for management and monitoring

\textbf{Application concepts}
\begin{itemize}
\item None at this level of granularity
\end{itemize}

\paragraph{Application development concepts and constraints}
\begin{itemize}
\item Pattern constraining application logic
\item Concepts for application development
\item Constraints for application development
\item Strategies for applicaion logic
\end{itemize}

\paragraph{Quality Attributes of Java-EE}
\begin{itemize}
\item Good Scalability
\item Good Reliability 
\item Average Flexibility
\item Average Performance
\item Auditability is not directly supported
\item Good Security
\item Quite good Integrability
\end{itemize}
...
\begin{itemize}
\item The framework allows a model-view-controller system of handling the interaction for users
\item It can be integrated into the lower layers of the Java EE reference system, allowing the mobile application and web pages
\item It allows easier to work with mySQL for handling the users and their documents
\end{itemize}
\subsubsection{MongoDB}
Another possibility, on top of mySQL, would be to use MongoDB. There are a few reasons for this, such as:
\begin{itemize}
\item It handles documents better, which could be used for the research papers
\item PHP does support MongoDB, so integrating it would be simple
\end{itemize}

\subsection{Access and integration channels}
\subsubsection{Integration requirements}

The main integration channel that will be used in the system is the database which will allow users to access to external systems either via the mobile application or the browser client. An EAI system can be used to participate in concurrent integration operations at any one time.
Security plays a vital role in the integration channels as unauthorised authors may not see, contribute or change documents while heads of departments or supervisors have the authority to see the papers being worked on.
The integration channel has to be reliable as papers that are being worked on are of utmost importance and would be detrimental to all those involved in the papers if something were to happen to them. The system must keep all documents secure and accounted for.

\subsubsection{Access channels}

There will be multiple access channels for the proposed system. Mobile/android application clients will be used to fetch data(such as the deadline dates) and integrate them into mobile device for users to easily be able to identify when their deadlines are  for. 
...
Web services will be used in order to store all the users and their details as well as all the papers that are being worked on by any particular user. These web services will be set up such that 100 users can concurrently access the server without any difficulties. 
...
\begin{itemize}
\item Linux - Operating system that will be used to host the website and back end of the service.
\item Apache - The web server.
\item MySQL - SQL database used to store data and will be queried for information.
\item PHP - Server scripting language used to interact.
\item MongoDB - NoSQL database which might be used on top of MySQL.
\item CodeIgniter - Lightweight MVC framework for rapid development of web applications.
\end{itemize}
...
\subsubsection{Android Mobile Application Technologies}
\begin{itemize}
\item Java EE - Programming language with APIs to be used.
\item GSON - Library for converting between Java Objects and JSON representations
\item RoboGuice - Dependency Injection framework for Android
\item Gradle - Build system to allow for more robust, well defined builds
\end{itemize}
There are 3 possible types of Android applications which can be built. For this project a native Android application will be built. Native applications offer the best performance and the best user interface compared to the other types. This is especially important when scalability is a factor and presenting the content correctly. As this will be a native application, Java will be used as the programming language.\newline\newline The GSON library will be used to convert between Java objects and JSON representations and visa versa which will be useful when communicating with the web server on campus.
