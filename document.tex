...
\subsection{Intergation requirements}



\subsection{Architecture constraints}

The server should be available at all times to allow all users to login at anytime. There should be search functionality to allow users to navigate faster.The system will be web-based and will also built to to run on mobile devices.

\subsubsection{Web-based development}

We will be building the web-based system using html, css, and some javascript for some user side functionality. We will do all client validation using javascript to check if what the user inputted is corect.  

\subsubsection{Server side development}

To run the database we will be using mySQL and php. We will store the user profile using a mySQL database and use, and all vaildation will be done in php.


\subsubsection{Mobile application development}

To build the mobile application, we will be using androidStudio(java). The mobile application should only be able to access the server when you are withing the premises of the University.
